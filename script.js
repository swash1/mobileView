...
function viewportHandler() {
  ...
  requestAnimationFrame(() => {
    ...
    info.innerHTML = window.visualViewport.offsetTop;

    // Stick to top
    wrapper.style.transform = `translateY(${Math.max(
      0,
    ...
    if (window.visualViewport.offsetTop >= 0) {
      bubbles.style.transform = `translateY(-${Math.max(
        40,
        window.innerHeight -
...
if (window.visualViewport) {
  window.visualViewport.addEventListener("resize", () =>
    viewportHandler(window.visualViewport.height)
  );
  ...
  window.addEventListener("resize", () => viewportHandler(window.innerHeight));
  window.addEventListener("scroll", () => viewportHandler(window.innerHeight));
}
